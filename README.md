...
臺北市見鼠通報熱點的結構性排序研究，以及對應的地圖介面。

用三組環境資料把臺北市切成格子並排出優先順序，回答的是**「有限人力該先去哪裡看」**，不是「哪裡有幾隻老鼠」。排序在封存之後才拿見鼠雷達的後續通報來檢驗，不能反過來。

---

## 這個專案在解決什麼

臺北市目前的鼠患處理是被動的：有人通報，才派人處理。人力有限，不可能每條巷弄都巡，所以資源實際上跟著抱怨聲跑——哪一區的居民比較會投訴，哪一區就先被處理，而不是哪裡問題比較嚴重。
...
> **見鼠雷達的通報只能在 T0 封存之後拿來驗證，不能當特徵、標籤、調權重、選模型、訓練或校準的依據。**
...
而 `sql/validation/11_rat_radar_layerwise_post_freeze_only.sql` 用資料庫層級的 `ASSERT` 強制執行——**沒有完成 T0 封存，驗證 SQL 直接跑不動**。

理由很簡單：如果先看答案再調分數，再拿同一批答案證明分數準，那什麼都沒證明。所以流程必須是「先封存、再觀察」。
